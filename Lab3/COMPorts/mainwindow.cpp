...
{
    sendedAmount += stuffedData.length() / 10;

    QString temp = stuffedData;
    int tIndex = temp.indexOf("@e");
    while(tIndex != -1) {
        temp.insert(tIndex, '\n');
        if(tIndex > 1)
            temp.insert(tIndex - 1, ' ');
        tIndex = temp.indexOf("@e", tIndex + 4);
    }
    // -5 to 5 bits FCS
    temp.insert(temp.length() - 1, ' ');

    QString hexData = QString("Порций отправлено: ") + QString::number(sendedAmount) + QString("\nСтруктура кадров после байт-стаффинга:") + temp;
    ...
    while(index != -1) {
        ...
        cursor.setPosition(index + 2, QTextCursor::KeepAnchor);

        cursor.mergeCharFormat(format);

        index = hexData.indexOf(sequence, index + 2);
    }
